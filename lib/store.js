...
export const useAuth = create((set, get) => ({
  ...
  restore: async () => {
    try {
      ...
      if (!data) {
        set({ status: "out", user: null, permissions: [], role: null });
        return false;
      }
      set({
        status: "in",
  ...
  logout: async () => {
    try {
      ...
      set({ status: "out", user: null, permissions: [], role: null, challengeToken: null });
    }
  },
...
if (typeof window !== "undefined") {
  api.session.onLost(() => {
    useAuth.setState({ status: "out", user: null, permissions: [], role: null });
  });
}
...
export const useData = create((set, get) => ({
  ...
  transitionOrder: (orderNo, payload) => api.orders.transition(orderNo, payload),
  refundOrder: (orderNo, amount, reason) => api.orders.refund(orderNo, amount, reason),
