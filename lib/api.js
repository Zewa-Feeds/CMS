...
function setSessionMarker(remember = false) {
  ...
  const maxAgeClause = remember ? `; Max-Age=${SEVEN_DAYS_SECONDS}` : "";
...
async function refreshSession() {
  refreshPromise ??= (async () => {
    try {
      ...
      // Re-assert marker with remember flag if server indicates remembered session
      const isRemembered = Boolean(data.isRemembered);
...
export const auth = {
  ...
  async verifyTwofa(challengeToken, code, remember = false) {
    ...
    session.set(data.accessToken, remember);
    return data;
  },
