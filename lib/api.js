...
const REFRESH_TOKEN_KEY = "zewa_cms_rt";
const REMEMBER_ME_KEY = "zewa_cms_remember";
 ...
 *
 * The real session is the httpOnly `zewa_rt` cookie and stored client token.
 * This cookie exists so middleware can redirect an unauthenticated visitor to
 * /login instead of serving them an admin shell that would immediately bounce.
 */
const SESSION_MARKER = "zewa_cms_session";
const SEVEN_DAYS_SECONDS = 7 * 24 * 60 * 60; // 604,800s

function setSessionMarker(remember = false) {
  if (typeof document === "undefined") return;
  const isRemembered =
    remember ||
    (typeof window !== "undefined" && localStorage.getItem(REMEMBER_ME_KEY) === "true");
  const expiresDate = new Date(Date.now() + SEVEN_DAYS_SECONDS * 1000).toUTCString();
  const maxAgeClause = isRemembered
    ? `; Max-Age=${SEVEN_DAYS_SECONDS}; Expires=${expiresDate}`
    : "";
  const secureClause = typeof location !== "undefined" && location.protocol === "https:" ? "; Secure" : "";
  document.cookie = `${SESSION_MARKER}=1; Path=/; SameSite=Lax${maxAgeClause}${secureClause}`;
}
...
export const session = {
  ...
  set(token, remember = false, refreshToken = null) {
    accessToken = token;
    const isRemembered =
      remember ||
      (typeof window !== "undefined" && localStorage.getItem(REMEMBER_ME_KEY) === "true");
    if (refreshToken) {
      storeRefreshToken(refreshToken, isRemembered);
    }
  ...
  /** Marker or stored refresh token presence */
  get exists() {
    return hasSessionMarker() || Boolean(getStoredRefreshToken());
  },
...
if (typeof window !== "undefined") {
  window.addEventListener("storage", (e) => {
    if (e.key === REFRESH_TOKEN_KEY && e.newValue) {
      setSessionMarker(localStorage.getItem(REMEMBER_ME_KEY) === "true");
    }
  });
}
...
async function refreshSession(attempt = 1) {
    ...
    try {
      ...
      const response = await fetch(`${ADMIN}/auth/refresh`, {
        ...
        headers: {
          ...
          ...(storedRt ? { "X-Refresh-Token": storedRt } : {}),
        },
        body: JSON.stringify(storedRt ? { refreshToken: storedRt } : {}),
        credentials: "include", // sends the httpOnly refresh cookie when supported
      });

      if (!response.ok) {
        // If server is restarting (502, 503, 504) or network glitch, retry up to 2 times
      ...
      // Re-assert marker and store updated rotated refresh token
      const isRemembered = Boolean(
        data.isRemembered ??
        (typeof window !== "undefined" && localStorage.getItem(REMEMBER_ME_KEY) === "true")
      );
      if (data.refreshToken) {
        storeRefreshToken(data.refreshToken, isRemembered);
      }
      setSessionMarker(isRemembered);
      return data;
    } catch {
...
export const auth = {
  ...
  async verifyTwofa(challengeToken, code, remember = false) {
    ...
    session.set(data.accessToken, remember, data.refreshToken);
    return data;
  },
  ...
  /** Restore a session on page load, using the httpOnly refresh cookie or stored refresh token. */
  async restore() {
  ...
  async logout() {
    ...
    try {
      await request("/auth/logout", {
        method: "POST",
        body: storedRt ? { refreshToken: storedRt } : {},
      });
    } finally {
      session.clear();
    }
  ...
  async changePassword(currentPassword, newPassword) {
    ...
    session.set(data.accessToken, true, data.refreshToken);
    return data;
  },
