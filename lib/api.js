...
export const orders = {
  ...
  /** Resend a customer order email (§6.3, §15). */
  async resendEmail(orderNo, emailId) {
    const { data } = await request(`/orders/${orderNo}/emails/${emailId}/resend`, {
      method: "POST",
    });
    return data;
  },
  ...
  async sendEmail(orderNo, { template, subject, heading, message, attachInvoice, toEmail }) {
    const { data } = await request(`/orders/${orderNo}/send-email`, {
      method: "POST",
      body: { template, subject, heading, message, attachInvoice, toEmail },
    });
    return data;
  },
};
...
export const customers = {
  /**
   * Re-issue a customer's email-verification link.
   *
   * Mints a FRESH token rather than resending the old email — the original is
   * single-use and 24-hour-bound, so replaying it would deliver a dead link.
   */
  async resendVerification(id) {
    const { data } = await request(`/customers/${id}/resend-verification`, { method: "POST" });
    return data;
  },
  ...
  async previewEmail({ heading, message, subject, ctaText, ctaUrl, customerName }) {
    const { data } = await request(`/customers/preview-email`, {
      method: "POST",
      body: { heading, message, subject, ctaText, ctaUrl, customerName },
    });
    return data;
  },
  /** Send bulk common or custom emails to clients/customers. */
  async sendEmail({ audience, customerIds, customEmails, subject, heading, message, ctaText, ctaUrl }) {
    const { data } = await request(`/customers/send-email`, {
      method: "POST",
      body: { audience, customerIds, customEmails, subject, heading, message, ctaText, ctaUrl },
    });
    return data;
  },
};
...
// ============================================================================
// Z-COIN LOYALTY (ZSOP004 §9.4)
//
// §9.4 marks these a LAUNCH requirement: "support and finance cannot operate
// the programme without them", and §13.1 puts the admin surface BEFORE the
// storefront so support can see a balance before a customer can spend one.
// ============================================================================

/**
 * Email log — every send attempt, order-related or not.
 *
 * Built after the Sep 2026 ZeptoMail outage: order mail was logged and resendable,
 * everything else was fire-and-forget, so the mail that actually failed was the
 * only mail with no record and no way to replay it.
 */
export const emails = {
  async list(params = {}) {
    const { data, meta } = await request(`/emails${qs(params)}`);
    return { data, meta };
  },

  /** Distinct template names, for the filter dropdown. */
  async templates() {
    const { data } = await request("/emails/templates");
    return data;
  },

  /** One row, including the stored HTML body. */
  async get(id) {
   ...
   * Resend one email.
   *
   * The server writes a NEW row linked to the original rather than mutating it,
   * so the failed attempt survives as incident history.
   */
  async resend(id) {
    const { data } = await request(`/emails/${id}/resend`, { method: "POST" });
    return data;
  },

  /** Resend several. The server de-duplicates and reports per-row outcomes. */
  async resendMany(ids) {
    const { data } = await request("/emails/resend", { method: "POST", body: { ids } });
    return data;
  },
};
...
export const loyalty = {
  /**
   * The customer list with balances.
   *
   * `filter` mirrors §9.4's named filters so campaigns and exceptions can be
   * pulled without an export: pending, expiring (within 30 days), negative,
   * flagged.
   */
  async customers(params = {}) {
    const { data } = await request(`/loyalty/customers${qs(params)}`);
    return data;
  },

  /**
   * The support panel — balances, every lot, and the full ledger.
   *
   * This is the screen that answers "why is my balance this number?" in 60
   * seconds, so it returns everything needed in one call rather than making
   * support click through three.
   */
  async customer(customerId) {
    const { data } = await request(`/loyalty/customers/${customerId}`);
    return data;
  },

  /**
   * Manual credit or debit (§9.2).
   *
   * `note` is mandatory and `approvedById` is required above the rule version's
   * threshold — both enforced server-side, so a client that forgets gets a 400
   * rather than an unexplained movement in a financial ledger.
   */
  async adjust(customerId, { coins, note, reason, approvedById }, { idempotencyKey } = {}) {
    const { data } = await request(`/loyalty/customers/${customerId}/adjust`, {
      method: "POST",
      body: { coins, note, reason, approvedById },
      /*
       * One key per SUBMISSION, not per attempt (§9.1). The caller generates it
       * once and reuses it on retry, so a double-click or a network retry
       * collides on the ledger's unique index instead of crediting twice.
  ...
  async freeze(customerId, reason) {
    const { data } = await request(`/loyalty/customers/${customerId}/freeze`, {
      method: "POST",
      body: { reason },
    });
    return data;
  ...
  /** Live liability, ageing by expiry bucket, and the exception list. */
  async liability() {
    const { data } = await request("/loyalty/liability");
    return data;
  },
   ...
   * The finance CSV (§9.4).
   *
   * Goes through `request()` rather than a plain <a href> so the Bearer token
   * is attached: the admin API authenticates on the Authorization header, and a
   * browser navigation carries none, so the link form answered 401.
   */
  async exportCsv() {
    const blob = await request("/loyalty/export", { raw: true });
    downloadBlob(blob, `zcoin-liability-${new Date().toISOString().split("T")[0]}.csv`);
  },

  /** Versioned configuration — a change writes a NEW version (§8.4 #35). */
  async rules() {
    const { data } = await request("/loyalty/rules");
    return data;
  },

  async updateRules(payload) {
    const { data } = await request("/loyalty/rules", { method: "PUT", body: payload });
    return data;
  },

  async reconcile() {
    const { data } = await request("/loyalty/reconcile", { method: "POST" });
    return data;
  },

  /**
   * Per-customer monthly earning-cap override (ZSOP004 §3.3).
   *
   * Returns the programme default, the customer's effective cap, and the full
   * override history — "what was this customer allowed, and when" is the audit
   * question, so revoked and expired rows come back too.
   */
  async earnCap(customerId) {
    const { data } = await request(`/loyalty/customers/${customerId}/earn-cap`);
    return data;
  },

  /** Set an override. Supersedes any live one; `reason` is mandatory. */
  async setEarnCap(customerId, { monthlyCapCoins, reason, expiresAt }) {
    const { data } = await request(`/loyalty/customers/${customerId}/earn-cap`, {
      method: "POST",
      body: { monthlyCapCoins, reason, ...(expiresAt ? { expiresAt } : {}) },
    });
    return data;
  },

  /** Revoke the live override, returning the customer to the default. */
  async clearEarnCap(customerId) {
    const { data } = await request(`/loyalty/customers/${customerId}/earn-cap`, {
      method: "DELETE",
    });
    return data;
   ...
   *
   * Returns the exception counts, the weekly high-return review list, and
   * `volume` — today's issuance and redemption against the trailing 7-day
   * average. Also carries `largeRedemptionsUnverified`: redemptions that WOULD
   * have needed OTP re-verification, which cannot run without an SMS provider.
   * Counting them keeps that gap a visible number rather than a silent absence.
   */
  async risk() {
    const { data } = await request("/loyalty/risk");
    return data;
  },
};
...
export const coupons = {
  ...
  /**
   * Admin only. Stops (or resumes) redemption without destroying the coupon —
   * its redemptions and revenue are the record of a promotion that ran, so
   * there is no DELETE to call.
   */
  async setActive(id, isActive) {
    const { data } = await request(`/coupons/${id}`, { method: "PATCH", body: { isActive } });
    return data;
  },
 ...
 *
 * An affiliate is a PROFILE attached to an ordinary coupon, so everything about
 * discounting still goes through /coupons and the one validation engine. These
 * endpoints manage the profile and read the attribution the orders recorded.
 */
export const influencers = {
  async list(params = {}) {
    return request(`/influencers${qs(params)}`);
  },
  async get(id) {
    const { data } = await request(`/influencers/${id}`);
    return data;
  },
  async create(body) {
    const { data } = await request("/influencers", { method: "POST", body });
    return data;
  },
  async update(id, body) {
    const { data } = await request(`/influencers/${id}`, { method: "PATCH", body });
    return data;
  },
  /** Stops future coupon use; every past order and figure is left alone. */
  async deactivate(id) {
    const { data } = await request(`/influencers/${id}/deactivate`, { method: "POST" });
    return data;
  },
  async activate(id) {
    const { data } = await request(`/influencers/${id}/activate`, { method: "POST" });
    return data;
  },
  /** Summary cards: orders, gross, discount given, net, average, first/latest. */
  async analytics(id, params = {}) {
    const { data } = await request(`/influencers/${id}/analytics${qs(params)}`);
    return data;
  },
  /** The attributed-orders table, read from each order's own snapshot. */
  async orders(id, params = {}) {
    return request(`/influencers/${id}/orders${qs(params)}`);
  },
};
...
/** Paise → "₹1,847" or exact decimals like "₹166.5" when non-integer. */
export function formatPaise(paise, { decimals } = {}) {
  const rupees = (paise ?? 0) / 100;
  const hasFraction = rupees % 1 !== 0;
  const showDecimals = decimals !== undefined ? decimals : hasFraction;
  return `₹${rupees.toLocaleString("en-IN", {
    minimumFractionDigits: decimals === true ? 2 : 0,
