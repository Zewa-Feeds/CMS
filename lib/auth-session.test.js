...
describe("remember me behavior", () => {
  it("initializes rememberMe to true by default", () => {
    expect(useAuth.getState().rememberMe).toBe(true);
  });

  it("updates rememberMe on login", async () => {
    await useAuth.getState().login("admin@zewafeeds.com", "pass", false);
    expect(useAuth.getState().rememberMe).toBe(false);

    await useAuth.getState().login("admin@zewafeeds.com", "pass", true);
    expect(useAuth.getState().rememberMe).toBe(true);
  });
});
